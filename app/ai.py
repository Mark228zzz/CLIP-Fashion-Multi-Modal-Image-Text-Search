...
def encode_all(df):
    ...
    for img_path, text in tqdm(zip(df.image, df.text)):
...
### ----- INIT model/tokenizer/datasets/device/indeces -----
...
model_path = os.path.join(BASE_DIR, '../models/clip_ft_best.pt')
...
val = pd.read_csv(os.path.join(BASE_DIR, '../data/val.csv'))

# Encode all validation dataset
IMG_EMBS_PATH = os.path.join(BASE_DIR, '../data/img_embs.npy')
TXT_EMBS_PATH = os.path.join(BASE_DIR, '../data/txt_embs.npy')

if not os.path.exists(IMG_EMBS_PATH) or not os.path.exists(TXT_EMBS_PATH):
    img_vectors, txt_vectors = encode_all(val)
    np.save(IMG_EMBS_PATH, img_vectors)
    np.save(TXT_EMBS_PATH, txt_vectors)
    print("Encoded and saved image/text embeddings.")
else:
    img_vectors = np.load(IMG_EMBS_PATH)
    txt_vectors = np.load(TXT_EMBS_PATH)
    print("Loaded cached image/text embeddings.")
